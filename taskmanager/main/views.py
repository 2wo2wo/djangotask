...
def index(request):
    student = Student.objects.get(s_user=request.user.id)
...
def cart(request):
    student = Student.objects.get(s_user=User.objects.get(pk = request.user.id))
    sum=0
    for i in student.programs_selected.all():
        sum+=i.price
    print(sum)
...
def removecart(request,cart_id):
    current_student = Student.objects.get(s_user=request.user.id)
    program = Program.objects.get(pk=cart_id)
    if request.user.is_authenticated:
        current_student.programs_selected.remove(program)
...
def addcart(request,cart_id):
    ...
    program = Program.objects.get(pk=cart_id)
    if request.user.is_authenticated:
        current_student.programs_selected.add(program)
        return HttpResponseRedirect(reverse('cart'))
    else:
        return render(request, 'main/log_in.html', {
        "message": "please login"
